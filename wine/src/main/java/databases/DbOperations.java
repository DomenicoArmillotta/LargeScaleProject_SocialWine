...
public class DbOperations {
    ...
    //puo essere parte della home
    public void homepageAdmin(String myUsername) throws Exception {
        ArrayList<Review> allReview = new ArrayList<>();
        int k = 0;
        ArrayList<User> users = new ArrayList<>(graph.showFollowedUsers(myUsername));
        if (users.size() != 0) {
    ...
    public void showMyAccount(String myUsername) {
        ...
        System.out.println("4.  See profile of a friends");
        Scanner scanSelection = new Scanner(System.in);
        String selection = scanSelection.nextLine();
        if (selection.equals("1")) {
            if (users.size() != 0) {
    ...
    public void show10User(ArrayList<User> users, int times, int perTimes) {
        int i = times * perTimes;
        if (users.size() == 0) {
            System.out.println("No user on the social");
        }
        for (i = times * perTimes; (i < users.size() && i < ((perTimes * times) + perTimes)); i++) {
            System.out.println("User to select " + i + " :");
            System.out.println("username = " + users.get(i).getUsername());
            System.out.println("user country = " + users.get(i).getCountry());
            System.out.println("user mail = " + users.get(i).getEmail());
            System.out.println("user twitter name = " + users.get(i).getTwitter_taster_handle());
            if (i != (users.size() - 1)) {
                System.out.println("------------------------------------------------");
            }
        }
    ...
    public void show10Comment(ArrayList<Review> comment, int times, int perTimes) {
        int i = times * perTimes;
        if (comment.size() == 0) {
            System.out.println("No review on the social");
        }
        for (i = times * perTimes; (i < comment.size() && i < ((perTimes * times) + perTimes)); i++) {
            System.out.println("Review to select " + i + " :");
            System.out.println("description = " + comment.get(i).getDescription());
            System.out.println("rating = " + comment.get(i).getRating());
            if (i != (comment.size() - 1)) {
                System.out.println("------------------------------------------------");
            }
        }
        ...
        public void usersMenuBanAdmin (String myUsername){
            ...
            if (selectioned.equals("X")) {
                ...
                if (selection.equals("1")) {
                    ...
                    try {
                        graph.deleteAllRelationFollow(users.get(convertedSelection).getUsername());
                    } catch (IndexOutOfBoundsException ecc) {

                    }
                    try {
                        graph.deleteAllRelationLike(users.get(convertedSelection).getUsername());
                    } catch (IndexOutOfBoundsException ecc) {

                    }
                    try {
                        graph.deleteAllRelationCreated(users.get(convertedSelection).getUsername());
                    } catch (IndexOutOfBoundsException ecc) {
